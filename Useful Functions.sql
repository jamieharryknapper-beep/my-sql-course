...
SELECT m.MessageId
    ...
    ,DATENAME(MONTH,m.ReceivedDate) AS ReceivedMonthDay
FROM Message m;
...
SELECT DATEFROMPARTS(2026, 3, 2) AS TheDate;

SELECT DATEFROMPARTS(DATEPART(YEAR, GETDATE()), DATEPART(MONTH, GETDATE()), 1) AS FirstOfTheCurrentMonth;

-- FORMAT returns a string given a data and a format specifier 
SELECT FORMAT(DATEFROMPARTS(2022, 1, 18), 'dd-MMM-yyyy');

SELECT FORMAT(DATEFROMPARTS(2022, 1, 18), 'dd MMMM yy');
