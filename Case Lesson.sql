...
SELECT CONCAT('Jamie',' ','Knapper') AS FullName
...
-- Use Sub-query then CTE to reduce duplication

SELECT
    hosp.Hospital AS Test
    ,hosp.NumberOfPatients
    ,hosp.NumberOfPatientsInSurgery
    ,ROUND(100.0 * hosp.NumberOfPatientsInSurgery / hosp.NumberOfPatients,2) AS PercentageOfPatientsInSurgery
FROM
    (
SELECT
        ps.Hospital
